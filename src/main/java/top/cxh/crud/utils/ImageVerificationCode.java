...
public class ImageVerificationCode {
     ...
     * @param code_length
     */
    public ImageVerificationCode(int weight,int height,int code_length) {
    	this.weight = weight;
    	this.height = height;
    ...
    public BufferedImage getImage() {
        ...
        for (int i = 0; i < code_length; i++) {           //画四个字符即可
            String s = randomChar() + "";      //随机生成字符，因为只有画字符串的方法，没有画字符的方法，所以需要将字符变成字符串再画
            sb.append(s);                      //添加到StringBuilder里面
            float x = i * 1.0F * weight / code_length;   //定义字符的x坐标
